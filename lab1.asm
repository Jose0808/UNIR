...
li $s1, 11             #le asigna un valor a $s1
...
blt $s0,$s1,END            #valida si $s0 es mayor que $s1 si no va a "END"
addi $s3,$s0, 0            #le asigna a $s3 el valor de $s0 que es el mayor en este caso
END:                       #crea la etiqueta END
IF1:                       #crea la etiqueta IF1
blt $s2,$s3,END1           #valida si $s2 es mayor que $s3 que es el mayor de los otros numeros si no va a "END"
